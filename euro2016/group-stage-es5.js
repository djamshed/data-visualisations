'use strict';
// converted group-stage-es6 to es5 thru babeljs.io
...
var updateDetails = function updateDetails(d) {
  d3.select('.chart').selectAll('.team__label').classed('active', false).filter(function (t) {
    return t.opponent === d.team || t.team === d.team;
  }).classed('active', true);
  activeTeam = d.team;
  var info = '<div style="font-size: 28px;">' + d.team + '</div>' + '<div style="margin: 5px 0;">Ranked <strong>' + getOrdinal(d.uefaRank) + '</strong> by UEFA, the team finished ' + '<strong>' + getOrdinal(d.groupStanding) + '</strong> in group ' + '<strong>' + d.group + '</strong> with ' + '<strong>' + d.pts + '</strong> points and ' + '<strong>' + diffFormatter(d.goalDiff) + '</strong> goal differential.</div>';
  var status = d.isOut ? '<div>' + d.team + ' is <strong>out of the competition</strong>.</div>' : '<div>' + d.team + ' has <strong>has advanced to Round of 16</strong>.</div>';
  var games = '<div style="margin: 5px 0;"></div>' + '<div><strong>Matches</strong>:</div>' + '<div>' + d.opponents.map(function (opp) {
    return opp.gameText;
...
var drawChart = function drawChart(data) {
  var widthScale = d3.scale.linear().domain(d3.extent(teams, function (d) {
    return d.uefaCoeff;
  })).range([70, 200]);
  ...
  // one row per team
  var teamEnter = d3.select('.chart').selectAll('.team').data(data).enter().append('div').attr('class', 'team');
  d3.select('.chart').selectAll('.team').data(data).classed('team--out', function (d) {
    return d.isOut;
  });
  // enter team name
  teamEnter.append('div').attr('class', 'team__label team__lead').on('mouseover', updateDetails);
  // update team name
  d3.select('.chart').selectAll('.team__lead').data(data).style('margin-bottom', function (d) {
    return sortBy === 'groups' && d.groupStanding === 4 ? '30px' : '0';
  }).attr('title', function (d) {
    return d.team + ' UEFA coefficient: ' + d.uefaCoeff + ' (' + getOrdinal(d.uefaRank) + ')';
  }).classed('active', function (d) {
    return d.team === activeTeam;
  }).text(function (d) {
    console.log(d.team);return d.team;
  });

  // enter oppponents
  teamEnter.append('div').style('display', 'inline-block').attr('class', 'team__opponents').selectAll('.opponent').data(function (d) {
    return d.opponents;
  }).enter().append('div').attr('class', 'opponent team__label');
  // update opponents
  d3.select('.chart').selectAll('.team').selectAll('.team__opponents').data(function (d) {
    return [d];
  }).selectAll('.opponent').data(function (d) {
    return d.opponents;
  }).classed('tie', function (d) {
    return d.gameWinner === '';
  }).classed('loss', function (d) {
    return d.gameWinner === d.opponent;
  }).classed('win', function (d) {
    return d.gameWinner !== '' && d.gameWinner !== d.opponent;
  }).style('width', function (d) {
    return widthScale(d.opponentUefaCoeff) + 'px';
  }).classed('active', function (d) {
    return d.opponent === activeTeam;
  }).attr('title', function (d) {
    return d.gameText;
  }).text(function (d) {
    return d.opponent;
...
// add click handlers
d3.selectAll('.sort-by').on('mousedown', function () {
  var btn = d3.select(d3.event.target);
  if (btn.classed('sort-by--opponents')) {
    teams.sort(function (a, b) {
      return a.groupCoeff - b.groupCoeff;
    });
  } else if (btn.classed('sort-by--group')) {
    teams.sort(function (a, b) {
      return a.group === b.group ? a.groupStanding - b.groupStanding : a.group > b.group ? 1 : -1;
    });
  }
  // activate button
